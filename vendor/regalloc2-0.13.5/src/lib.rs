...
use core::{hash::BuildHasherDefault, iter::FromIterator, ptr};
...
// `bumpalo::Bump` does not implement `allocator_api2::alloc::Allocator` when
// the `allocator_api` feature is enabled. Implement the allocator behavior
// directly using the bump arena's public allocation API.
unsafe impl allocator_api2::alloc::Allocator for Bump {
    ...
    unsafe fn grow(
        ...
        let slice = ptr::slice_from_raw_parts_mut(new_ptr.as_ptr(), new_layout.size());
    ...
    unsafe fn grow_zeroed(
        ...
        let slice = ptr::slice_from_raw_parts_mut(new_ptr.as_ptr(), new_size);
        Ok(core::ptr::NonNull::new_unchecked(slice))
    }
