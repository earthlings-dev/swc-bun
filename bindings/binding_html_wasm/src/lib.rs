    ...
    fn minify_css(
        ...
        match &*opts {
            ...
            CssMinfierOptions::Swc(options) => {
                ...
                let mut css_gen = swc_css_codegen::CodeGenerator::new(wr, options.codegen);

                match mode {
                    CssMinificationMode::Stylesheet => {
                        swc_css_codegen::Emit::emit(&mut css_gen, &stylesheet).unwrap();
                    }
                    CssMinificationMode::ListOfDeclarations => {
                        ...
                        swc_css_codegen::Emit::emit(&mut css_gen, &block).unwrap();
                    ...
                    CssMinificationMode::MediaQueryList => {
                        ...
                        swc_css_codegen::Emit::emit(&mut css_gen, &prelude).unwrap();
...

                {
                    ...
                    match document_or_document_fragment {
                        DocumentOrDocumentFragment::Document(document) => {
                            html_gen.emit(&document).context("failed to emit")?;
                        }
                        DocumentOrDocumentFragment::DocumentFragment(document_fragment) => {
                            html_gen
                                .emit(&document_fragment)
